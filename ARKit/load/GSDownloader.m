...
NSString * const GSBlobDownloadErrorDomain = @"com.thibaulGSha.GSblobdownload";
NSString * const GSBlobDownloadErrorHTTPStatusKey = @"GSBlobDownloadErrorHTTPStatusKey";
@interface GSDownloader ()
// Public
...
@property (nonatomic, assign, readwrite) GSBlobDownloadState state;
// Download
...
- (void)finishOperationWithState:(GSBlobDownloadState)state;
                   ...
                   delegate:(id<GSBlobDownloaderDelegate>)delegateOrNil
{
    ...
    if (self) {
        ...
        self.state = GSBlobDownloadStateReady;
...
- (void)start
{
    ...
    if (![NSURLConnection canHandleRequest:self.fileRequest]) {
        NSError *error = [NSError errorWithDomain:GSBlobDownloadErrorDomain
                                             code:GSBlobDownloadErrorInvalidURL
    ...
    if (self.connection && ![self isCancelled]) {
        ...
        self.state = GSBlobDownloadStateDownloading;
...
- (BOOL)isExecuting
{
    return self.state == GSBlobDownloadStateDownloading;
}

- (BOOL)isCancelled
{
    return self.state == GSBlobDownloadStateCancelled;
}

- (BOOL)isFinished
{
    return self.state == GSBlobDownloadStateCancelled || self.state == GSBlobDownloadStateDone || self.state == GSBlobDownloadStateFailed;
}
...
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    if (httpResponse.statusCode >= 400) {
        error = [NSError errorWithDomain:GSBlobDownloadErrorDomain
                                    code:GSBlobDownloadErrorHTTPError
                                userInfo:@{ NSLocalizedDescriptionKey:[NSString stringWithFormat:@"Erroneous HTTP status code %ld (%@)",
                                                                       (long) httpResponse.statusCode,
                                                                       [NSHTTPURLResponse localizedStringForStatusCode:httpResponse.statusCode]],
                                            GSBlobDownloadErrorHTTPStatusKey: @(httpResponse.statusCode) }];
    }
    
    long long expected = @(self.expectedDataLength).longLongValue;
    if ([GSDownloader freeDiskSpace].longLongValue < expected && expected != -1) {
        error = [NSError errorWithDomain:GSBlobDownloadErrorDomain
                                    code:GSBlobDownloadErrorNotEnoughFreeDiskSpace
...
- (void)cancel
{
    ...
    [self finishOperationWithState:GSBlobDownloadStateCancelled];
...
- (void)notifyFromCompletionWithError:(NSError *)error pathToFile:(NSString *)pathToFile
{
    ...
    GSBlobDownloadState finalState = success ? GSBlobDownloadStateDone : GSBlobDownloadStateFailed;
